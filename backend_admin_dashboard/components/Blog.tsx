...
export default function Blog({
  ...
  // Fetch blog data if id is provided
  useEffect(() => {
    const blogId = id || _id;
    if (blogId && !existingTitle) {
      const fetchBlog = async () => {
        try {
          const response = await axios.get(`/api/blogs?id=${blogId}`);
          const blogData = response.data;
          if (blogData) {
            setTitle(blogData.title || "");
            setSlug(blogData.slug || "");
            setBlogcategory(blogData.blogcategory || []);
  ...
  async function createProduct(ev: React.FormEvent) {
    ...
    if (blogId) {
      await axios.put("/api/blogs", { ...data, _id: blogId });
    ...
    <form onSubmit={createProduct} className="addWebsiteform">
      ...
      <div className="w-100 flex flex-col flex-left mb-2" data-aos="fade-up">
        <label htmlFor="catergory">
          Select Category (ctrl + leftclick for multiple select)
        </label>
        <select
          onChange={(e) =>
            setBlogcategory(
