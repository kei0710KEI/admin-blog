...
export async function generateMetadata({
  ...
  const url = absoluteUrl(`/tag/${tags}`);
  const title = `タグ: ${tags}`;
  const description = `${tags}に関する記事一覧`;
...
export default async function TagPage({ params }: PageProps) {
  ...
  const blogs = (await Blog.find({ tags: tags }).lean()) as any[];
        ...
        <div className="container">
          <div className="category_title">
            <div className="flex gap-1" data-aos="fade-right">
              ...
              <span>
                {
                  publishedblogs.filter((blog) => blog.tags.includes(tags))
                    .length
                }
              </span>
            </div>
